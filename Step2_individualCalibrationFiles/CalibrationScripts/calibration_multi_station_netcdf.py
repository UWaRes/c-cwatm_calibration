...
Adapted by Augustin Clédat for the GERICS/UWARES Team : Now the process can read a netcdf with a very precise shape 
...
grdc = grdc[['id','calibrated','OBJECTID','runoff_mean','gauged_lon','gauged_lat']]
grdc.load()
...
if regional_id=='None' :
    ...
    # stations_with_nan=list(grdc.id.where(grdc.OBJECTID==int(regional_id)).where(grdc.calibrated==1).values)
    stations=[int(regional_id)]
...
streamflow_data =grdc.sel(id=stations).runoff_mean.sel(time=slice(ForcingStart, ForcingEnd))
...
gauges = ''
for i in range(len(grdc.sel(id=stations).id)) :
	gauges += '{} {} '.format(grdc.sel(id=stations).gauged_lon[i].values, grdc.sel(id=stations).gauged_lat[i].values) 
